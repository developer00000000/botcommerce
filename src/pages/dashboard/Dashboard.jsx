...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <h1 className="text-gray-800 text-2xl mb-6">Dashboard</h1>

      <div className="bg-white p-6 rounded-lg shadow-lg mb-6">
        <div className="flex space-x-4">
          <div>
            <h4 className="text-gray-800 text-md">From</h4>
            <DatePicker
              selected={startDate}
              onChange={(date) => setStartDate(date)}
              className="mt-1 w-full p-2 bg-gray-200 text-gray-800 rounded border border-gray-300"
              placeholderText="mm/dd/yyyy"
            />
          </div>
          <div>
            <h4 className="text-gray-800 text-md">To</h4>
            <DatePicker
              selected={endDate}
              onChange={(date) => setEndDate(date)}
              className="mt-1 w-full p-2 bg-gray-200 text-gray-800 rounded border border-gray-300"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {[
          ...
          >
            <span className="text-3xl">{card.icon}</span>
            <h3 className="text-gray-500 text-sm mt-4">{card.label}</h3>
            <p className="text-gray-800 text-2xl mt-2">{card.value}</p>
          </div>
        ))}
      </div>

      <div className="p-6 rounded-lg mb-6 mt-[100px] w-[350px]">
        <label className="text-lg mb-2">General</label>
        <select
          value={timeframe}
          onChange={(e) => setTimeframe(e.target.value)}
          className="mt-5 w-full p-2 text-gray-800 rounded-lg border border-gray-400 h-[50px]"
        >
          <option>Daily</option>
          <option>Weekly</option>
          <option>Monthly</option>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {[
          ...
          >
            <span className="text-4xl mb-4">📄</span>
            <h3 className="text-gray-500 text-lg mb-2">{card.label}</h3>
            <p className="text-gray-600">{card.message}</p>
          </div>
        ))}
      </div>
      <SallingProducts />
